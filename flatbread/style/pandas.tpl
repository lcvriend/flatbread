{# pandas template for research purposes #}
{# https://github.com/pandas-dev/pandas/blob/master/pandas/io/formats/templates/html.tpl #}
...
{% block table %}
    <table
        id="T_{{uuid}}"
        {% if table_attributes %} {{table_attributes}}{% endif %}>
    ...
    {% block tbody %}
        ...
        {% block before_rows %}{% endblock before_rows %}
            ...
            {% for c in r %}
                {% if c.is_visible != False %}
                <{{c.type}}
                    {% if c.id is defined -%}id="T_{{uuid}}{{c.id}}"{%- endif %}
                    class="{{c.class}}" {{c.attributes|join(" ")}}
                >{{c.display_value}}</{{c.type}}>
                {% endif %}
            {% endfor %}
            </tr>
            {% endblock tr %}
